class CropElement:
    ...
    def __init__(
        ...
        self.source_image = source_image  # Исходное изображение (ресайзнутое до кратного размера кропам)
        ...
        self.detected_xyxy_real = None  # Список списков с координатами xyxy боксов в значениях от source_image
        self.detected_masks_real = None # Список np массивов с масками в случае yolo-seg размером как source_image
    ...
    def calculate_real_values(self):
        ...
        self.detected_xyxy_real = []  # List of lists with xyxy box coordinates in the values ​​of the source_image
        self.detected_masks_real = []  # List of np arrays with masks in case of yolo-seg sized as source_image
        ...
        if self.detected_masks is not None:
            ...
            for mask in self.detected_masks:
                ...
                black_image = np.zeros_like(self.source_image)
                ...
                black_image[y_start_global:y_start_global+img.shape[0], x_start_global:x_start_global+img.shape[1]] = mask_resized
